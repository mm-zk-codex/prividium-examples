...
async function deploy() {
  ...
  const provider = new ethers.JsonRpcProvider({
    url: process.env.RPC_URL || 'https://api.prividium.dev/rpc',
    headers: {
...
- **RPC URL**: The base proxy RPC endpoint (e.g., `https://api.prividium.dev/rpc`)
...
const prividium = createPrividiumChain({
  ...
  prividiumApiBaseUrl: process.env.PERMISSIONS_API_BASE_URL || 'https://api.prividium.dev',
...
**Important:** The wallet must be configured with the `/rpc/wallet/{token}` RPC URL for write operations. This ensures
transactions are routed through Prividium's authenticated rpc.
